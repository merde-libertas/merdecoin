 ...
 */
static const char* BITCOIN_PID_FILENAME = "merdecoind.pid";
 ...
 * between merdecoind, and merdecoin-qt and non-server tools.
*/
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("merdecoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/merdecoin/merdecoin>";
    const std::string URL_WEBSITE = "<https://merdecoincore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("merdecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Merdecoin is running in a usable environment with all
...
{
    // Make sure only a single Merdecoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if merdecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if merdecoin is started while in a temporary directory.\n",
