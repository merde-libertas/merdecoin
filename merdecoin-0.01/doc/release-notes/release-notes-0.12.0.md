Merdecoin Core version 0.12.0 is now available from:

  <https://merdecoin.org/bin/merdecoin-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Merdecoin Core or other software:
...
ECDSA signatures inside Merdecoin transactions now use validation using
[libsecp256k1](https://github.com/merdecoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/merdecoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Merdecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Merdecoin Core was to raise the default minimum
relay fee.

Merdecoin Core 0.12 will have a strict maximum size on the mempool. The
...
Merdecoin Core 0.12 also introduces new default policy limits on the length and
...
Merdecoin Core 0.12 nodes. Merdecoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/merdecoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Merdecoin Core 0.12 does not yet have support for
...
This allows running merdecoind without having to do any manual configuration.
...
Merdecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Merdecoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Merdecoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Merdecoin Core 0.13, and it is at this time undecided whether the
...
Merdecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Merdecoin Core automatically creates a hidden service to listen on, without
manual configuration. Merdecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Merdecoin Core is listening, and
...
Merdecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Merdecoin
Core to use floating fees. By default, Merdecoin Core will use floating
fees.
...
(default: `0.0002` MRD/kB).

At all times, Merdecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MRD.
Furthermore, Merdecoin Core will never create transactions paying less than
...
`merdecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/merdecoin/merdecoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/merdecoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/merdecoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /merdecoinrpc>
        ...
        # optional bypass merdecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< merdecoinrpc:<password>
    </Location>
...
Restarting merdecoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Merdecoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new merdecoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include merdecoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix merdecoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of merdecoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [merdecoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libmerdecoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "MRD" (Ross Nicoll)
