Merdecoin Core version 0.18.0 is now available from:

  <https://merdecoincore.org/bin/merdecoin-core-0.18.0/>
...
`/Applications/Merdecoin-Qt` (on Mac) or `merdecoind`/`merdecoin-qt` (on
Linux).
...
Merdecoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Merdecoin Core on unsupported systems.
...
10.10. Additionally, Merdecoin Core does not yet change appearance when
...
`contrib/init/merdecoind.service`, it has been changed to use
`/var/lib/merdecoind` as the data directory instead of
`~merdecoin/.merdecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/merdecoind` will
exist has enough space (check using `df -h /var/lib/merdecoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/merdecoin/merdecoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://merdecoincore.org/en/contact/) the Merdecoin Core
...
The systemd init file (`contrib/init/merdecoind.service`) has been changed
to use `/var/lib/merdecoind` as the data directory instead of
`~merdecoin/.merdecoin`. This change makes Merdecoin Core more consistent with
...
`datadir` option in `/etc/merdecoin/merdecoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/merdecoin/merdecoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/merdecoin/merdecoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/merdecoin/merdecoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/merdecoin/merdecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/merdecoin/merdecoin/blob/master/doc/merdecoin-conf.md)
  about the `merdecoin.conf` file describes how to use it to configure
  Merdecoin Core.

- A new document introduces Merdecoin Core's BIP174 [Partially-Signed
  Merdecoin Transactions
  (PSBT)](https://github.com/merdecoin/merdecoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/merdecoin/merdecoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Merdecoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Merdecoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/merdecoin/merdecoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/merdecoin/merdecoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Merdecoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Merdecoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/merdecoin/merdecoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/merdecoin/merdecoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart merdecoind with the `-deprecatedrpc=generate`
...
- A new `merdecoin-wallet` tool is now distributed alongside Merdecoin
...
This section describes planned changes to Merdecoin Core that may affect
other Merdecoin software and services.

- Since version 0.16.0, Merdecoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Merdecoin Core 0.20 (expected about
  a year after 0.18), Merdecoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Merdecoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Merdecoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Merdecoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Merdecoin Core
...
- A sub-project of Merdecoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Merdecoin Core.  See their [project
  page](https://github.com/merdecoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Merdecoin Core's own implementation, although entropy
  gathered by Merdecoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Merdecoin Core a
...
- On macOS, Merdecoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `merdecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libmerdecoin_server.a` (jnewbery)
...
- #13926 [Tools] merdecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like merdecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `merdecoin-qt -version` output formatting (hebasto)
...
- #11625 Add MerdecoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libmerdecoin_server` (laanwj)
...
- #15154 Configure: merdecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/merdecoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Merdecoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libmerdecoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in merdecoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Merdecoin Core (Christewart)
...
- #12246 Bugfix: Only run merdecoin-tx tests when merdecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_merdecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_merdecoin` (MarcoFalke)
...
- #14186 merdecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_merdecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update merdecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/merdecoin-conf.md (hebasto)
...
- #15354 Add missing `merdecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct merdecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support merdecoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/merdecoin/).
