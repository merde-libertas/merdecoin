# RapidCheck property-based testing for Merdecoin Core

## Concept

Property-based testing is experimentally being added to Merdecoin Core with
...
If RapidCheck is installed, Merdecoin Core will automatically run the
...
3. Configure Merdecoin Core with RapidCheck.

    `cd` to the directory of your local merdecoin repository and run
...
4. Build Merdecoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_merdecoin-crypto_gen.o
    CXX      test/test_merdecoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Merdecoin Core.
