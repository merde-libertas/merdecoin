...
const char * const BITCOIN_CONF_FILENAME = "merdecoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "merdecoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //merdecoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "merdecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Merdecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Merdecoin
    // Mac: ~/Library/Application Support/Merdecoin
    // Unix: ~/.merdecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Merdecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Merdecoin";
#else
    // Unix
    return pathRet / ".merdecoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Merdecoin Core copyright is not removed by accident
    if (copyright_devs.find("Merdecoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Merdecoin Core developers";
    }
